...
네이버에서, 이후에는 <http://zetablog.tistory.com>의 티스토리 블로그로 다양한 글을
...
사실 <https://github.com/nielsenramon/chalk>에서 시키는대로 하면 다 된다! 그런데 이 테마에서는 기본적으로 $$\LaTeX$$ 지원이 되지를 않아, 이를 설정해주기 위해...
...
으로 jekyll-latex를 설치하고, Gemfile에도 추가하였는데 블로그에 변함이 없다. 이후 \_config.yml에도 추가해봤지만, 여전히 변함이 없다. 이후 만지작거리다가 Dependency Error가 생겨, Google에 검색을 하니 <https://stackoverflow.com/questions/35401566/dont-have-jekyll-paginate-or-one-of-its-dependencies-installed>의 내용을 발견했다. 나도 마찬가지로 두 버전의 jekyll이 설치돼 있어서 최신 버전만 남겨 두었다. 그 과정에서 오타가 생겼는지,
...
<https://helloworldpark.github.io/jekyll/update/2016/12/18/Github-and-Latex.html>를 참고하였다.
...
간단한 Googling을 통해 <https://github.community/t5/GitHub-Pages/Github-forgets-about-my-custom-domain-every-time-I-update-the/td-p/470>를 찾았다.
...
Disqus 댓글 지원은 <https://xho95.github.io/blog/jekyll/disqus/migration/2017/01/20/Add-Disqus-to-Jekyll.html>을 참고했다.
